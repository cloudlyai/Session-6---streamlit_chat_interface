# Session 6 - Streamlit Chat UI
  ...
  Take the puzzle agent from Session 4 (terminal-based) and give it a browser chat UI using Streamlit. The agent    
  logic stays the same, only the UI layer changes.

  ## Programs

  ### `streamlit_agent.py`
  The main puzzle agent with a Streamlit chat interface:
  ...
  - **Rendering order:** capture input, append to `chat_history`, render history, run agent loop, append response,  
  `st.rerun()`
